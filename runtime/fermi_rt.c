...
void fe_print_bool(int v)       { fputs(v?"true":"false", stdout); }
void fe_print_i32(int32_t v)    { printf("%d", v); }
void fe_print_i64(int64_t v)    { printf("%lld", (long long)v); }
void fe_print_f32(float v)      { printf("%g", (double)v); }
void fe_print_f64(double v)     { printf("%g", v); }
void fe_print_char(char v)      { putchar((unsigned char)v); }
void fe_print_str(const char *s){ if(s) fputs(s, stdout); }
void fe_println_bool(int v)     { fputs(v?"true":"false", stdout); putchar('\n'); }
void fe_println_i32(int32_t v)  { printf("%d\n", v); }
void fe_println_i64(int64_t v)  { printf("%lld\n", (long long)v); }
void fe_println_f32(float v)    { printf("%g\n", (double)v); }
void fe_println_f64(double v)   { printf("%g\n", v); }
void fe_println_char(char v)    { printf("%c\n", (unsigned char)v); }
void fe_println_str(const char *s){ if(s) puts(s); else putchar('\n'); }
void fe_println_void(void)      { putchar('\n'); }
void fe_flush(void)             { fflush(stdout); }
...
char *fe_replace(const char *s, const char *from, const char *to) {
    ...
    size_t ls=strlen(s);
    char *r=malloc(ls+count*(lt>lf?lt-lf:0)*100+1);
    char *w=r; p=s;
    const char *f;
    while ((f=strstr(p,from))) {
        size_t before=f-p; memcpy(w,p,before); w+=before;
...
char *fe_slice(const char *s, int start, int end) {
    ...
    if (start<0) start=0; if (end>len) end=len; if (start>end) start=end;
    int n=end-start;
    char *r=malloc(n+1); memcpy(r,s+start,n); r[n]='\0'; return r;
}
int fe_parse_int(const char *s) { return s?atoi(s):0; }
float fe_parse_float(const char *s) { return s?(float)atof(s):0.0f; }
char *fe_int_to_str(int32_t v) { char buf[32]; snprintf(buf,sizeof(buf),"%d",v); return strdup(buf); }
char *fe_float_to_str(float v) { char buf[64]; snprintf(buf,sizeof(buf),"%g",(double)v); return strdup(buf); }

int32_t fe_abs(int32_t v)         { return v<0?-v:v; }
float   fe_absf(float v)          { return v<0?-v:v; }
float   fe_sqrt(float v)          { return sqrtf(v); }
float   fe_powf(float b, float e) { return powf(b,e); }
float   fe_floor(float v)         { return floorf(v); }
float   fe_ceil(float v)          { return ceilf(v); }
float   fe_round(float v)         { return roundf(v); }
...
float   fe_sin(float v)            { return sinf(v); }
float   fe_cos(float v)            { return cosf(v); }
float   fe_tan(float v)            { return tanf(v); }
float   fe_log(float v)            { return logf(v); }
float   fe_log2f(float v)          { return log2f(v); }
float   fe_log10f(float v)         { return log10f(v); }
...
void   fe_exit(int32_t code)              { exit(code); }
...
char *fe_read_file(const char *path) {
    ...
    char *buf=malloc(sz+1); size_t r=fread(buf,1,sz,f); (void)r; buf[sz]='\0'; fclose(f); return buf;
}
void fe_write_file(const char *path, const char *content) {
    FILE *f=fopen(path,"wb"); if(!f) return;
    if (content) fwrite(content,1,strlen(content),f); fclose(f);
}
...
int64_t fe_array_pop(FeArray *a) {
    if (!a || a->len <= 0) return 0;
    return a->data[--a->len];
}
int64_t fe_array_get(FeArray *a, int64_t idx) {
    if (!a || idx < 0 || idx >= a->len) return 0;
    return a->data[idx];
}
void fe_array_set(FeArray *a, int64_t idx, int64_t val) {
...
typedef struct FeMapEntry {
    char *key; int64_t val;
    struct FeMapEntry *next;
} FeMapEntry;
...
void fe_map_delete(FeMap *m, const char *key) {
    ...
    while (*pp) {
        if (strcmp((*pp)->key, key)==0) {
            FeMapEntry *e = *pp; *pp = e->next; free(e->key); free(e); return;
        }
        pp = &(*pp)->next;
    }
}
void fe_map_free(FeMap *m) {
    ...
    for (int i=0; i<FE_MAP_BUCKETS; i++) {
        ...
        while (e) { FeMapEntry *n = e->next; free(e->key); free(e); e = n; }
    }
    free(m);
}
