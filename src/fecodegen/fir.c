...
/* Print a value: constants and globals printed as-is, SSA names get % prefix */
static void pv(const char *v) {
    if (!v || !v[0]) { printf("undef"); return; }
    if (v[0]=='%'||v[0]=='@') { printf("%s",v); return; }
    /* numeric constant (including negative) */
...
static void fir_print_instr(FirInstr *i) {
    ...
    /* ── Comparison ─────────────────────────────────────────── */
    case FIR_ICMP:
        /* %dst = icmp <pred> <ty> %src1, %src2  (pred in src3) */
        ...
        break;
    case FIR_FCMP:
        /* %dst = fcmp <pred> <ty> %src1, %src2  (pred in src3) */
    ...
    case FIR_PTRTOINT: case FIR_INTTOPTR: case FIR_BITCAST: {
        ...
        /* %dst = <cast> <from_ty> %src1 to <to_ty>  (from_ty in src2) */
    ...
    case FIR_CALL_INDIRECT: {
        ...
        /* function name: user fns get @, already-prefixed names kept */
