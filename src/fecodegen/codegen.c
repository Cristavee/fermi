...
static void var_def(Codegen *cg, const char *name, const char *type, const char *slot) {
  ...
  /* Heap-copy a string value: strlen(v)+1 -> malloc -> strcpy -> return heap ptr */
  ...
  /* Free and heap-copy: free(old) then emit_mut_str_copy(new) */
...
/* io::print(val) — the only io function. newline is manual: io::print("hello\n") */
static char *emit_io_print(Codegen *cg, AstList *args) {
      if (!args) return arena_strdup(cg->arena,"0");
      AstNode *anode = args->node;
      /* Interpolated string already emits printf per segment — no wrapper needed */
      ...
      /* pick format string based on arg type */
      const char *fmt;
...
static char *emit_expr(Codegen *cg, AstNode *n) {
    ...
    case NODE_BINARY: {
        ...
        if (strcmp(ast_type_str(cg,n->binary.left),"ptr")==0 &&
            (!strcmp(op,"+") || !strcmp(op,"-"))) {
            if (!cg->in_unsafe) {
                fprintf(stderr,"%d:%d: error[codegen]: pointer arithmetic requires an unsafe block\n",
                        n->line,n->col);
    ...
    case NODE_ASSIGN: {
          ...
          if (lhs->kind==NODE_IDENT) {
              ...
              if (e) {
                  if (e->is_mut && strcmp(e->type,"ptr")==0 && strcmp(rty,"ptr")==0) {
                      /* mut string reassignment: free old heap, malloc+strcpy new value */
        ...
        if (lhs->kind==NODE_DEREF) {
            ...
            if (!cg->in_unsafe) {
                fprintf(stderr,"%d:%d: error[codegen]: raw pointer dereference-assign requires an unsafe block\n",
                        n->line,n->col);
            }
    ...
    case NODE_CALL: {
        ...
        }
        {
            ...
            /* Route alloc()/realloc() to region-aware variants */
            ...
            /* Runtime builtin return type lookup — prevents i32 default fallback */
            ...
            fn_name=actual_fn;
            /* Coerce args to match callee param types (e.g. i32 literal → i64 param) */
            for(FirFn *fn=cg->mod.fns;fn;fn=fn->next) {
    ...
    case NODE_AT_CALL: {
        ...
        fprintf(stderr,"%d:%d: error[codegen]: unknown intrinsic '@%s'\n",n->line,n->col,_an);
        n->ty=TY_VOID; return arena_strdup(cg->arena,"0");
    }
    case NODE_NS_CALL:
    ...
    case NODE_CAST: {
        ...
        if (strcmp(from,"ptr")==0 && is_int_type(to)) {
            if (!cg->in_unsafe) {
                fprintf(stderr,"%d:%d: error[codegen]: pointer-to-integer cast requires an unsafe block\n",
                        n->line,n->col);
                return arena_strdup(cg->arena,"0");
            }
            char *dst=newtmp(cg);
        ...
        if (is_int_type(from) && strcmp(to,"ptr")==0) {
            if (!cg->in_unsafe) {
                fprintf(stderr,"%d:%d: error[codegen]: integer-to-pointer cast requires an unsafe block\n",
                        n->line,n->col);
                return arena_strdup(cg->arena,"null");
            }
            char *dst=newtmp(cg);
    ...
    case NODE_DEREF: {
        ...
        if (!cg->in_unsafe) {
            fprintf(stderr,"%d:%d: error[codegen]: raw pointer dereference requires an unsafe block\n",
                    n->line,n->col);
        }
    ...
    case NODE_INTERP_STR: {
          /* Emit each part as a printf call — no heap allocation needed.
             Parts alternate: NODE_STRING_LIT (literal segment) / expr (value).
             Literal segments use %s, expressions use type-appropriate format. */
...
static void emit_stmt(Codegen *cg, AstNode *n) {
    ...
    switch (n->kind) {
    case NODE_VAR_DECL: {
          /* If no explicit type annotation, infer from the init expression */
          int is_mut = n->var_decl.is_mut;
          const char *ty;
          if (n->var_decl.type) {
              ty = typenode_to_llvm(cg, n->var_decl.type);
          } else if (n->var_decl.init) {
              /* Emit init first to set its ty field, then infer type */
              char *v=emit_expr(cg,n->var_decl.init);
              const char *vty=ast_type_str(cg,n->var_decl.init);
              ty = vty ? vty : "i32";
              /* mut string: heap-copy so it can be modified */
              if(is_mut && strcmp(ty,"ptr")==0) v=emit_mut_str_copy(cg,v);
              char slot[128]; snprintf(slot,sizeof(slot),"var.%s",n->var_decl.name?n->var_decl.name:"_");
