...
static int is_integral(TypeKind k){
    /* bool (i1) counts as integral — valid for bitwise ops like (flags & FLAG_READ) */
...
static void terr(Tc *t,int line,int col,const char *msg){
    fprintf(stderr,"%d:%d: error[type]: %s\n",line,col,msg);
    t->had_error=1;
}
static void def_builtins(Tc *t){
    static const char *names[]={
        "print","println","input","malloc","free","sizeof","len","concat",
        "to_upper","to_lower","trim","trim_left","trim_right","contains",
        "starts_with","ends_with","replace","index_of","slice","split",
        "parse_int","parse_float","int_to_str","float_to_str",
        "abs","absf","sqrt","pow","floor","ceil","round","min","max",
        "minf","maxf","clamp","sin","cos","tan","asin","acos","atan",
        "atan2","log","log2","log10","exp","fabs","fmod","fmin","fmax","hypot",
        "alloc","realloc","memcopy","memset","memcmp","exit","getenv","setenv",
        "args","time","sleep","getcwd","chdir","read_file","write_file","flush",
        "eprint","eprintln","rand","srand",
        "strcmp","strlen","strcpy","strcat","strstr","atoi","atof","sprintf","snprintf","sscanf",
        "array_new","array_push","array_pop","array_get","array_set","array_len","array_free",
        "map_new","map_set","map_get","map_has","map_delete","map_free",
        "Ok","Err",NULL
    };
...
/* Handled by existing tc.c — NODE_EXPORT_MODULE is ignored in tc */
