...
static void err(Parser *p, const char *msg){
    fprintf(stderr,"%d:%d: error[syntax]: %s, got '%s'\n",
        p->cur.line, p->cur.col, msg, p->cur.value);
    p->had_error=1;
}
...
static AstNode *parse_primary(Parser *p){
    ...
    if(chk(p,TOK_STRING_LIT)){
        ...
        /* Check if string contains %{ — if so, parse as interpolated string */
        int has_interp=0;
        ...
        /* Build NODE_INTERP_STR by splitting on %{...} */
        ...
        while(*src){
            ...
            /* Always emit the literal segment (even if empty — marks position) */
            {
            ...
            if(!*src) break;  /* end of string */
            src+=2; /* skip %{ */
            /* Extract expression source up to matching } */
            ...
            if(*src=='}')src++; /* skip } */
            /* Parse the expression using a sub-parser on the extracted source */
            char *expr_src=(char*)arena_alloc(p->arena,expr_len+1);
            memcpy(expr_src,expr_start,expr_len); expr_src[expr_len]='\0';
            Parser sub=parser_new(expr_src,expr_len,p->arena);
...
static AstNode *parse_do_while(Parser *p){
    ...
    mat(p,TOK_FAT_ARROW); /* optional => before while */
...
static AstNode *parse_match_arm(Parser *p){
    ...
    if(chk(p,TOK_IDENT)&&strcmp(p->cur.value,"_")==0){
        ...
        /* bool literal match arm: true => 1, false => 0 */
...
static int is_fn_name_tok(Parser *p){
    /* Allow any identifier-like token as a function name, including keyword repurposed as method name */
    switch(p->cur.type){
...
static AstNode *parse_fn_decl(Parser *p, int is_unit, int is_extern, int is_exported){
    ...
    if(is_extern){
        if(chk(p,TOK_LBRACE)){
            /* external fn foo() {} — skip empty body, treat as declaration */
            adv(p); /* consume '{' */
            if(chk(p,TOK_RBRACE)) adv(p); /* consume '}' if immediately closed */
            else { /* non-empty body: skip until matching } */
                int depth=1;
                while(!chk(p,TOK_EOF)&&depth>0){
...
Parser parser_new(const char *src, size_t src_len, Arena *arena){
    Parser p;
